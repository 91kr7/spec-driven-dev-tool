<!--
  ...
  Authority: conventions [§3](../conventions.md#3-spec-front-matter) + scot.md (the ONLY behavioral grammar — branch ids [Bn], arm enumeration, [§6](../scot.md#6-error-style) error style).
...
owns_sections: []
error_style: <result|raise>  # behavioral only (scot.md §6) — canonical home for error style
---
...
# Public interface
<List every public method; neutral types (scot.md [§2](../scot.md#2-lexical-conventions)); list errors regardless of error_style.>
...
# Invariants & rules
<List properties that always hold + business rules enforced; reference deps by id.>
- <Invariant / rule, traceable to a requirement>

# SCoT
<One FUNCTION per public method, grammar per scot.md. Give every branch a stable [Bn] id; enumerate ALL its arms. No library/syntax/SQL — those go to impl-notes.>
...
# Acceptance criteria
<Each `ACn` as Given/When/Then; cover the happy path AND every branch arm (≥1 test per ACn and per arm — scot.md [§7](../scot.md#7-branch-id-rules)).>
