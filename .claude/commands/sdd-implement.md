...
5. **Read the latest `.sdd/state.md` record** for that scope (per §6 format) and decide:
   ...
   - **REJECT** → route by the verdict's `routing` field, incrementing the iteration against the **code budget (3)**:
     ...
     - `routing: spec-writer` (the gate blames the spec — spec is wrong/ambiguous, MD is authority) → ESCALATE-via-handoff: the spec must be re-reviewed before code can be trusted. Re-invoke `spec-writer` with the reasons, then the spec must pass `analysis-gatekeeper` again (run/re-run `/sdd-specify` for that id — that demotes/re-advances its `status`); only then resume `code-implementer` for it. Spec rework does **not** consume the code budget.
