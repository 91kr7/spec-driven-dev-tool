...
6. **Consistency** — every `depends_on`/`CALL <id>`/component reference resolves; the index `source` column matches the spec's `source:`; ids stable.
