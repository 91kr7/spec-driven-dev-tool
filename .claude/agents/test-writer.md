...
## Inputs
- `.claude/sdd/conventions.md` — coverage-id rule, [§6](../sdd/conventions.md#6-verdict-records) verdict so you know what's checked.
- `scot.md` — [§4](../sdd/scot.md#4-control-constructs) arm ids, [§6](../sdd/scot.md#6-error-style) error style, [§7](../sdd/scot.md#7-branch-id-rules) coverage contract.
- `ui-schema.md` — [§5](../sdd/ui-schema.md#5-events-table-and-journey-acs) Events + journey ACs, Accessibility.
...
9. **Coverage id** — tag each test with its canonical id (scot.md [§7.3](../sdd/scot.md#7-branch-id-rules): `<spec-id>::<function>#<arm-id>` or `<spec-id>#ACn`, `#AC` never `::AC`) **verbatim in a leading comment** (source of truth for matching) and, where possible, in the test name/tag.
