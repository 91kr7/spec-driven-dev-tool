...
MISSION: Produce tests that are an INDEPENDENT ORACLE derived only from the behavioral part of the specs — ≥1 test per `ACn` + per SCoT branch arm — so the suite judges behavioral equivalence, not implementation detail.
...
NON-GOALS (never do):
- NEVER read `src/` or the `.sdd/impl-notes/` tree. Sibling dir under `.sdd/`, NOT under `.sdd/specs/` — glob `.sdd/specs/**/*.spec.md` only, never `*.impl-notes.md`.
...
## Inputs
- `.claude/sdd/conventions.md` — coverage-id rule, [§6](../sdd/conventions.md#s6) verdict so you know what's checked.
- `scot.md` — [§4](../sdd/scot.md#s4) arm ids, [§6](../sdd/scot.md#s6) error style, [§7](../sdd/scot.md#s7) coverage contract.
- `ui-schema.md` — [§5](../sdd/ui-schema.md#s5) Events + journey ACs, Accessibility.
- `.sdd/target.md` — test framework(s), canonical commands, `tests/` layout, **its §2 language-idioms map** (= calling convention to derive call sites from).
...
9. **Coverage id** — tag each test with its canonical id (scot.md [§7.3](../sdd/scot.md#s7): `<spec-id>::<function>#<arm-id>` or `<spec-id>#ACn`, `#AC` never `::AC`) **verbatim in a leading comment** (source of truth for matching) and, where possible, in the test name/tag.
10. **Self-check** before hand-off:
    - every in-scope **test-covered** `ACn` + SCoT arm has ≥1 test (a `(pipeline)` AC has none — covered by its canonical command);
...
- Nothing informed by `src/`/`.sdd/impl-notes/`.
- Suite is syntactically runnable.
- **Every test file carries ≥1 real assertion and maps to a coverage id** — no zero-assertion, placeholder, or naming-breadcrumb files; every filename legal for the language.
...
- On a test-bug REJECT, command passes verdict `reasons[]` + prior `test_paths`. **Read the offending test; fix with a minimal edit** so it correctly asserts its `ACn`/arm — never regenerate the whole suite for one test bug.
- **Un-testable unit:** if an AC/branch cannot be tested without naming an implementation detail (spec ambiguous/inconsistent), do NOT invent behavior or leave a silent gap. Write a **failing marker test** tagged with the unit's coverage id whose only assertion fails with `SPEC DEFECT: <spec-id>#ACn | <spec-id>::fn#arm — <reason>`. Unit is covered but failing, so the gatekeeper triages it as a **spec bug → spec-writer**.
