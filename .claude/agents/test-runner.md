...
2. **Resolve `{scope}`** from scope ids (per `target.md` §2 layout — one target per spec id), in the form each command expects (file globs, or id-alternation `--grep`/`--tests`). No scope ids → run whole suite (empty `{scope}` / `test-all`). Record resolved scope (or `whole-suite (unscoped)`). Fill **only** `{scope}`; never add/alter any other flag.
