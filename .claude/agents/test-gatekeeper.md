...
5. **Coverage check.** If any `ACn` or any branch arm in scope has NO mapped test → REJECT, route `test-writer`, naming each uncovered unit by its fully-qualified id. A unit whose **only** test is **skipped/ignored** (`it.skip`/`xit`/`@Ignore`/`@Disabled` — counted under `skipped` in the REPORT) counts as **uncovered**: REJECT, route `test-writer` (or `spec-writer` if the skip flags a spec defect).
   - **E2E journey coverage (GUI projects only — `.sdd/target.md` Frontend framework ≠ `none`).** For each in-scope gui **screen** spec (`CLS-*`, `kind: gui`), confirm there is ≥1 **Playwright e2e** test (in `suites: … e2e`) covering its **primary success journey**, plus one per **end-to-end failure journey the screen renders** (e.g. a service-error banner). A screen whose journeys are covered **only** by in-process **component** tests (the feature mocked) is **not** e2e-covered → REJECT, route `test-writer`, naming the screen id and the missing journey. (Arm-level / accessibility / pure-view ACs may remain covered by component tests; this clause adds the journey-level e2e on top, mirroring the ui-schema §9 baseline mandate. Non-GUI projects have no gui screens → this clause is inert.)
...
- For a **GUI project**, an in-scope gui **screen** spec has NO **Playwright e2e** test for its primary journey (or a rendered failure journey) — covered only by component tests → route `test-writer`.
