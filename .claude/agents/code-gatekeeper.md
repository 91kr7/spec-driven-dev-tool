...
2. **Spec-untouched** — confirm implementer did NOT edit the gated spec (§8). Use `git diff`/`git log` read-only on `.sdd/specs/**` if available. Altered spec → immediate REJECT routed `spec-writer`.
3. **Mapping reality**
...
4. **Traceability header + comment economy**
   ...
   - **Header is the only mandatory comment (§13).** Flag **egregious re-narration** — a file whose comments paraphrase the spec/SCoT/ACs line-by-line, Purpose docstrings, section banners, "what" comments on self-evident code → REJECT route `code-implementer`. Judge gross duplication of the spec, not tasteful one-offs; a single non-obvious rationale line is fine.
...
9. **Build/lint check (read-only)**
   - Run `target.md`'s non-mutating build/lint command to confirm scoped files compile. `install` ran at step 6b so the toolchain is present — **this is how the gate backs §5's _compiles_**, so run by default (not optional).
