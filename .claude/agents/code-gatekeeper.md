...
4. **Mapping reality.** For every path in `source:`, verify the file exists on disk (Glob/Read). A declared-but-missing file is a REJECT. Then sweep `src/` (Glob) for files NOT claimed by any spec's `source:` — an extra/orphan source file with no owning spec is a REJECT (route to spec-writer if it should be specced, else code-implementer if it is stray). **Ignore tool-generated / VCS-ignored paths** when detecting orphans: consult `.gitignore` and skip standard generated dirs (`node_modules/`, `dist/`, `build/`, `.next/`, `coverage/`, caches, compiled output) — only hand-authored, un-ignored files count as orphans.
