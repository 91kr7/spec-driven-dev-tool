...
## Inputs
- `.claude/sdd/conventions.md` (front-matter [§3](../sdd/conventions.md#3-spec-front-matter), status [§5](../sdd/conventions.md#5-status-lifecycle-and-separation-of-duties), verdict [§6](../sdd/conventions.md#6-verdict-records), change policy [§8](../sdd/conventions.md#8-change-policy), headers [§13](../sdd/conventions.md#13-traceability)), `scot.md` (the SCoT contract), `ui-schema.md` (for `kind: gui`), `.sdd/target.md` (stack, read-only build/lint commands, header comment syntax).
...
2. **Spec-untouched** — confirm implementer did NOT edit the gated spec ([§8](../sdd/conventions.md#8-change-policy)). Use `git diff`/`git log` read-only on `.sdd/specs/**` if available. Altered spec → immediate REJECT routed `spec-writer`.
3. **Mapping reality**
...
4. **Traceability header + comment economy**
   - Each comment-supporting `source:` file carries its [§13](../sdd/conventions.md#13-traceability) header (after any mandatory first-line construct).
   - Comment-less formats (pure JSON, lockfiles) are exempt — verify mapping from `source:` alone.
   - **Header is the only mandatory comment ([§13](../sdd/conventions.md#13-traceability)).** Flag **egregious re-narration** — a file whose comments paraphrase the spec/SCoT/ACs line-by-line, Purpose docstrings, section banners, "what" comments on self-evident code → REJECT route `code-implementer`. Judge gross duplication of the spec, not tasteful one-offs; a single non-obvious rationale line is fine.
...
9. **Build/lint check (read-only)**
   - Run `target.md`'s non-mutating build/lint command to confirm scoped files compile. `install` ran at step 6b so the toolchain is present — **this is how the gate backs [§5](../sdd/conventions.md#5-status-lifecycle-and-separation-of-duties)'s _compiles_**, so run by default (not optional).
...
- a comment-supporting file lacks its [§13](../sdd/conventions.md#13-traceability) header;
- a file egregiously re-narrates the spec in comments ([§13](../sdd/conventions.md#13-traceability) comment economy);
...
## Hand-off
- Write exactly one verdict file `.sdd/verdicts/<scope>/code.md` ([§6](../sdd/conventions.md#6-verdict-records) format + economy; OVERWRITE), `phase: code`.
