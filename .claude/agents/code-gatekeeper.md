...
5. **Traceability header.** Each source file in `source:` must carry its §13 header pointing back to the spec id + path, in the target language's comment syntax. A missing/incorrect header is a REJECT.
...
- A path declared in a spec's `source:` is missing on disk, OR an extra/orphan source file exists under `src/` that no spec's `source:` claims.
- A source file in `source:` lacks (or has an incorrect) §13 traceability header.
