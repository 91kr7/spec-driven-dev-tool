...
6. **Schema (MOD-build only)** — generate each forward script at its declared `source:` path **from the corresponding `ENT-*` field table** (resolved via `MOD-build.depends_on`); forward-only — read shipped scripts + current field table, emit a **new `Vn` for the delta only**, never edit a shipped script. If a needed script has no `source:` entry → a spec gap: stop, leave it to the spec phase (no orphan file).
