...
3. **NEW file → generate from spec.** Create the file in the target language/framework (`.sdd/target.md`), translating the body form: SCoT for behavioral kinds, the field table for entities, signatures-only for interfaces, the component tree for `gui`. Stamp the **traceability header** at the top of the file (comment syntax per target), e.g. `// spec: CLS-regCtrl RegistrationController — specs/classes/CLS-regCtrl.spec.md` — but place it **after** any mandatory first-line construct (shebang `#!`, `<?php`, a `"use client"` directive, an XML/encoding decl), and **omit it entirely for comment-less formats** (pure JSON like `package.json`/`tsconfig.json`, lockfiles), whose spec↔source link is the `source:` declaration alone (§13).
...
5. **Map the SCoT faithfully — not 1:1.** Realize `sequence / branch / loop`, `TRY/CATCH`, and `ASYNC/AWAIT` per `.claude/sdd/scot.md`; honor the declared `error_style` (`result`→target result/Either idiom, `raise`→target exceptions) and map every `LOG`/`EMIT`/`ASSERT` intent to a concrete mechanism. Enforce all stated invariants and pre/post-conditions. For entities, emit the field table with its types/relations/constraints; **DB schema changes live under `MOD-build`, derived from the entity specs** — never hand-author schema changes independently. When you implement `MOD-build`, generate each schema-change script (at its declared `source:` path) **from the corresponding `ENT-*` field table/invariants** (resolved via `MOD-build`'s `depends_on` — the persistence module(s) and the entities they contain) — the entity spec is the schema's single source; never add a column the entity does not declare. Schema changes are **forward-only and incremental**: read the already-shipped scripts under `MOD-build`'s `source:` plus the current `ENT-*` field table and emit a **new forward script for the delta only** (the next `Vn`) — **never edit an already-shipped script**. The new script's path must already be declared in `MOD-build`'s `source:` (the spec-writer adds it); if a needed script has no `source:` entry, that is a spec gap — stop and leave it to the spec phase, do **not** create an orphan file. Resolve each `CALL <id>.method(...)` to the real symbol from that id's `source:`.
6. **Reuse, never duplicate.** Emit each shared abstraction (`SHR-*`) and shared UI component (`COMP-*`) **once**, at its declared `source:` path, and **import it** everywhere it is referenced by id. Discover-before-create: if a needed abstraction already exists, import it; do not re-implement it. For a co-owned aggregator file, write only the section named in this spec's `owns_sections:`. **Escape hatch for un-specced shared logic:** if you need shared logic that has **no `SHR-*`/`COMP-*` spec yet**, do NOT create a new shared file (it would be flagged as an orphan) and do NOT silently scatter copies. Inline it **minimally** at the current spec's declared `source:` path to keep moving, and **record it in `.sdd/impl-notes/<id>.md` as a promotion candidate** (what / where / why) so the reuse-analyst promotes it to a real `SHR-*`/`COMP-*` next cycle and the spec-writer specs it — the code-gatekeeper routes such a flagged, not-yet-specced duplication to `spec-writer`, not back to you.
