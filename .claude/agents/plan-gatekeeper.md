...
## Inputs
- `.claude/sdd/conventions.md` (rules), `.sdd/target.md`, `.sdd/REQUIREMENT.md`, `.sdd/PLAN.md`, `.sdd/specs/` indexes (existing project only — read the rows' **ids + `depends_on`** lazily, for id-stability (step 2), the whole-project DAG (step 3), and consumer sets (step 6); empty/ignored on a NEW project).
...
3. **DAG (whole-project)** — the `depends_on` graph is acyclic. On existing-SDD the plan is a **delta**, so build the graph from the **existing index rows ∪ the delta's edges** (a NEW/MODIFY edge can close a cycle with an unchanged, out-of-delta entity); an interface-break must re-point members' edges so no cycle remains (name the cycle members).
